        ...
        if(preg_match($skpattern, $buffer, $regs))
        {
            ...
            if(isset($db_list{$sk_file_name}))
            {
                ...
                $db_list{$sk_file_name}{'size'} = 
                        $db_list{$sk_file_name}{'size'} .'->'.$sk_file_size;
                $db_list{$sk_file_name}{'sum'} = 
                        $db_list{$sk_file_name}{'sum'} .'->'."md5 $sk_file_md5 " . "sha1 $sk_file_sha1";
                $db_list{$sk_file_name}{'changed'} = 1;
            }
            else
            {
                $db_list{$sk_file_name}{'changed'} = 0;
                $db_list{$sk_file_name}{'time'} = $time_stamp;
                $db_list{$sk_file_name}{'size'} = $sk_file_size;
    ...
    foreach($db_list as $list_name => $list_val)
    {
        ...
        /*$entry = array(
            $list_name => array(
                ...
                'md5' => $list_val['md5'],
                'sha1' => $list_val['sha1'],
                'oldSize' => $list_val['oldSize'],
                'newSize' => $list_val['newSize'],
                'sum' => $list_val['sum']
            )
        );*/
        $entry = array(
