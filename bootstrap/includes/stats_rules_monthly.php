<table id="rules_table_monthly" class="table table-striped table-bordered" cellspacing="0" width="100%">
	<thead>
		<tr>
		<th>Rule</th>
    	<th>Value</th>
	...
	if( array_key_exists( 'rule', $monthly_stats ) ) {
		foreach($monthly_stats{'rule'} as $l_rule => $v_rule)
		{
			(int)$rule_pct = (int)($v_rule * 100)/$monthly_stats{'alerts'};
			if(($odd_count % 2) == 0)
			{
				$odd_msg = ' class="odd"';
			}
	...
	<td>'.number_format($monthly_stats{'alerts'}).'</td>
	<td>100%</td>
	</tr>
	';?>
	</tbody>
...
<script type="text/javascript" src="./includes/tools/jquery.dataTables.min.js"></script>
<script type="text/javascript" src="./includes/tools/jquery.dataTable.percentageBars.js"></script>
<script type="text/javascript" src="./includes/tools/dataTables.buttons.min.js"></script>
<script type="text/javascript" src="./includes/tools/buttons.print.min.js"></script>
...
$(document).ready(function() {
  ...
  var table = $('#rules_table_monthly').DataTable({
	...
	var buttons = $("#rules_table_monthly_wrapper .dt-buttons");
	buttons.addClass("btn-group my-3");
	var filter= document.querySelector("#rules_table_monthly_filter");
	filter.className= "input-group col-sm-2";
	filter.children[0].children[0].className="form-control";
	var show= document.querySelector("#rules_table_monthly_length");
